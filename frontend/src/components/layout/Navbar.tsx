...
import { BookOpen, Search, Upload, Settings, LogOut, Sun, Moon, Bell, Check, MessageSquare, BookPlus, List } from "lucide-react";
...
// Color per notification type (for the icon dot)
function notifIconColor(type: string): string {
  if (type === "reply_to_my_comment") return "#818cf8";        // indigo
  if (type === "new_comment_on_my_book") return "#34d399";    // green
  if (type === "new_edition_on_my_book") return "#fb923c";    // orange
  if (type === "book_added_to_list") return "#60a5fa";         // blue
  return "#9ca3af";
}
...
function NotificationPopup({
  ...
  return (
    <div
      className="absolute right-0 top-full mt-2 w-96 rounded-xl shadow-2xl z-50 overflow-hidden border"
      style={{
        background: "var(--notif-bg, #ffffff)",
        borderColor: "var(--notif-border, #e5e7eb)",
      }}
    >
      {/* Header */}
      <div
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={onClose}
            className="text-xs"
            style={{ color: "var(--notif-muted, #9ca3af)" }}
          >
            Close
          </button>
        </div>
      ...
      <div className="overflow-y-auto" style={{ maxHeight: "min(480px, 70vh)" }}>
        {notifications.length === 0 ? (
          <div className="px-4 py-10 text-center">
            ...
            <p className="text-sm" style={{ color: "var(--notif-muted, #9ca3af)" }}>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    backgroundColor: isUnread
                      ? "var(--notif-unread-bg, rgba(224, 233, 255, 0.55))"
                      : "transparent",
                  }}
                  onMouseEnter={(e) => {
                    (e.currentTarget as HTMLElement).style.backgroundColor =
                      "var(--notif-hover-bg, #f9fafb)";
                  }}
                  onMouseLeave={(e) => {
                ...
                >
                  ...
                  <div
                    ...
                    style={{
                      backgroundColor: notifIconColor(n.type) + "22",
                      color: notifIconColor(n.type),
                    }}
                  >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {n.detail && (
                      <p
                        className="text-xs mt-0.5 line-clamp-1 italic"
                        style={{ color: "var(--notif-muted, #9ca3af)" }}
                      >
                        "{n.detail}"
                      </p>
                  ...
                  {/* Unread dot */}
                  {isUnread && (
                    <div className="w-2 h-2 rounded-full bg-ink-500 flex-shrink-0 mt-1.5" />
                  )}
                </button>
              );
            })}
...
export function Navbar() {
  ...
  const markRead = useMutation({
    ...
    // Optimistic update: flip read_at immediately in the cache
    onMutate: async (ids) => {
  ...
  const handleMarkOne = useCallback(
    (id: number, link: string) => {
      // Mark this notification read, then navigate
      markRead.mutate([id]);
      setNotifOpen(false);
      navigate(link);
    },
  ...
  return (
    <>
      {/* CSS variables for notification popup -- scoped so they only affect the popup */}
      <style>{`
        html:not(.dark) {
          --notif-bg: #ffffff;
          --notif-border: #e5e7eb;
          --notif-divider: #f3f4f6;
          --notif-heading: #0f172a;
          --notif-text: #1e293b;
          --notif-muted: #9ca3af;
          --notif-unread-bg: rgba(224, 233, 255, 0.55);
          --notif-hover-bg: #f9fafb;
        }
        html.dark {
          --notif-bg: #1a1f2e;
          --notif-border: #2d3748;
          --notif-divider: #252d3d;
          --notif-heading: #f0f4f8;
          --notif-text: #e2e8f0;
          --notif-muted: #718096;
          --notif-unread-bg: rgba(30, 58, 95, 0.5);
          --notif-hover-bg: #1e2535;
        }
      `}</style>
              ...
              <>
                ...
                <div className="relative" ref={notifRef}>
                  ...
                  >
                    ...
                    {hasUnread && (
                      <span className="absolute -top-0.5 -right-0.5 min-w-[1.1rem] h-4 bg-red-500 text-white rounded-full text-[10px] font-bold flex items-center justify-center px-0.5 leading-none">
